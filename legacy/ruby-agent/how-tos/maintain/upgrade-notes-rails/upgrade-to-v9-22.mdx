...
A [smart relationship](/legacy/ruby-agent/reference-guide/models/relationships/create-a-smart-relationship/overview) that only looks a record up by key runs its block once per row. The agent cannot optimize it. When the relation can be described by a key, declare a real `belongs_to` or `has_one` on the model instead. The agent then joins it, or preloads it for the whole page, and serves it like any other relation (filter, sort, related data).
...
```ruby Before — lib/forest_liana/collections/car.rb (Forest customization)
class Forest::Car
  include ForestLiana::Collection

  collection :Car

  belongs_to :pilot, reference: 'Driver.id' do
    Driver.find_by(firstname: object.model)
  end
end
```

```ruby After — lib/forest_liana/collections/car.rb (Forest customization)
class Forest::Car
  include ForestLiana::Collection
...
```ruby After — app/models/car.rb (application code)
class Car < GarageRecord
  belongs_to :pilot, class_name: 'Driver',
             primary_key: :firstname, foreign_key: :model,
             optional: true
end
```
...
This works even when the target lives in **another database**. Such a relation cannot be joined, so the agent preloads it: on a list of 100 records, it runs 2 queries instead of 101. Use `primary_key:` when the key is not the target's primary key. Restart the agent afterwards to regenerate the schema.

<Warning>
The association becomes part of your model, so the rest of your application sees it too. Since Rails 5, a `belongs_to` is **required by default**: without `optional: true`, saving a record whose foreign key is empty or matches no target fails validation everywhere in your application, not only in Forest. Also check that the name you choose does not collide with an existing method of the model.
</Warning>

{/* TODO: set the version that ships ForestAdmin/forest-rails#816 */}
<Info>
The preload of cross-database `belongs_to` and `has_one` relations on lists, CSV exports and Rails 6.1 related lists ships in forest_liana **9.22.x**. On earlier 9.22 versions, these relations still load once per row.
</Info>

</Step>
<Step title="Keep the foreign key in association scopes — application code">

An association whose scope narrows its `select` must keep the key that links it to its owner. Otherwise it cannot be preloaded (see [check 6](#6-check-the-associations-you-will-preload)):
