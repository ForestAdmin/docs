...
To upgrade, **update the version in your Gemfile** to at least 9.22.1:

```ruby
gem 'forest_liana', '~> 9.22.1'
```

<Warning>
Do not stay on 9.22.0: a smart field declaring a multi-hop `dependencies:` path (such as `address:city:name`) answers **500** on the whole list when the list also displays the first relation of the path. 9.22.1 fixes it.
</Warning>
...
A [smart relationship](/legacy/ruby-agent/reference-guide/models/relationships/create-a-smart-relationship/overview) cannot declare what it reads, so a request that displays one falls back to `SELECT *` for the whole page, and its block runs once per row. When the relation can be described by a foreign key, declare a real `belongs_to` or `has_one` on the model instead. Every list that displays it then gets column selection back, the agent loads the relation with the page's query instead of one query per row, and it can be filtered and sorted.
...
<Info>
Keep this step to associations whose target lives in the **same database** and whose key is the target's primary key. On forest_liana 9.22.1, a relation to another database, or one declaring a `primary_key:` of its own, is still loaded once per row.
</Info>

{/* TODO: once ForestAdmin/forest-rails#816 is released, document the preload of cross-database belongs_to / has_one relations and of relations declaring a primary_key:, with its version. A cross-database relation is not filterable or sortable (SchemaUtils.disable_filter_and_sort_if_cross_db!). */}
