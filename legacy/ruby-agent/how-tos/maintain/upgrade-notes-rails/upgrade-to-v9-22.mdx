...
## Adapt your application code to get the full benefit
...
<Steps>
<Step title="Declare dependencies: on your smart fields">
...
Once a field is declared, run [check 4](#4-check-model-code-that-reads-columns-the-page-does-not-display) again on its collection, then look for `read the "..." column without declaring it` warnings in your logs: each one names a column that is missing from the declaration.

</Step>
<Step title="Replace lookup smart relationships with real associations">
...
```ruby Before (lib/forest_liana/collections/car.rb)
class Forest::Car
  include ForestLiana::Collection
...
This works even when the target lives in **another database**. Such a relation cannot be joined, so the agent preloads it: on a list of 100 records, it runs 2 queries instead of 101. Use `primary_key:` when the key is not the target's primary key, and `optional: true` when the foreign key can be empty. Remove the smart relationship declaration so that only the real association is left, then restart the agent to regenerate the schema.
...
<Step title="Keep the foreign key in association scopes">
...
<Step title="Disable the count of your largest collections">

If you overrode a count controller to call `deactivate_count_response` on a whole collection, replace the override with the collection option:
...
6. Once the upgrade is stable, [adapt your application code](#adapt-your-application-code-to-get-the-full-benefit) on your heaviest collections.
