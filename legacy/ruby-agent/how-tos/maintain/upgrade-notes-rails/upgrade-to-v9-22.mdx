...
<Step title="Keep the foreign key in association scopes — application code">

An association whose scope narrows its `select` must keep the key that links it to its owner. Otherwise, as soon as a `dependencies:` path names it, the preload raises `ActiveModel::MissingAttributeError` and the request answers **500** (see [check 6](#6-check-the-associations-you-will-preload)):
