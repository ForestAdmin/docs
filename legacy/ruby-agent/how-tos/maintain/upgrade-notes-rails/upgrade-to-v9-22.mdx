...
| --- | --- |
| A field you explicitly ask for through a relation (`fields[...]`), in a list, a record, an update response or a related list | **403** — `You are not allowed to read '<path>' from the '<Collection>' collection.` |
| A related field the interface expands by default (to-one relations) | Silently **removed** from the response |
...
- Listing or counting related records requires the **browse** permission on the related collection itself. A role that could open a parent record and see a related list without browse on the related collection will now get a **403** there.
...
**How to detect it**: in your development or staging environment, log in with each non-admin role and open the collections, related data sections, filters, sorts and searches your teams use daily. A 403 with one of the messages above means the role is missing a permission it was implicitly relying on.
...
Lists of collections that declare **no smart field** now select only the columns the request asks for, plus the primary key, the foreign keys of the relations in the request, and the STI type column. Any Ruby code that runs on these records and reads another column raises `ActiveModel::MissingAttributeError`, which answers **500**.
...
None of these are required. They are the features this release adds; adopting them is incremental.
...
- The field keeps working exactly as before. A request that displays a smart field with no `dependencies:` keeps the previous `SELECT *` behavior for that collection, and a getter reaching a relation keeps its one-query-per-row cost.
...
| Related data section | No permission check on the related collection | **browse** required on the related collection (and **edit** / **delete** on associate, update, dissociate) |
...
| List of a collection without smart fields | `SELECT *` | Only the requested columns, keys and STI column |
...
- `Cannot retrieve the "..." value of the "..." collection because of an internal error in the getter implementation: ...` — the getter raised twice; its value is served as `nil`.
...
5. Watch the logs above and the 403/422/500 rate of your agent for a few days.
