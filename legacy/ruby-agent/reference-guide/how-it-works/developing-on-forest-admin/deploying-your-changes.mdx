...
To achieve this, you'll be using Forest CLI's [deploy](/legacy/ruby-agent/reference-guide/how-it-works/developing-on-forest-admin/forest-cli-commands/deploy) command from an environment **that has your reference environment as its origin**:
...
<Warning>
If you want to deploy from an environment whose origin is not your reference environment, you'll need to use the [set-origin](/legacy/ruby-agent/reference-guide/how-it-works/developing-on-forest-admin/forest-cli-commands/set-origin) environment first so you can make it its origin.
</Warning>

<Warning>
Don't forget to **deploy your backend changes** as well (if any), as showcased on [this flowchart](/legacy/ruby-agent/reference-guide/how-it-works/developing-on-forest-admin/overview#development-workflow).
</Warning>
...
`push` means moving your branch's changes to a remote environment set as origin of your branch. To achieve this, you'll be using Forest CLI's [push](/legacy/ruby-agent/reference-guide/how-it-works/developing-on-forest-admin/forest-cli-commands/push) command:

```
forest push
```

Note that you'll be pushing your **current** branch. To select another branch, use [switch](/legacy/ruby-agent/reference-guide/how-it-works/developing-on-forest-admin/forest-cli-commands/switch). If the origin of your branch is not the remote you want (ie: staging) change it with [set-origin](/legacy/ruby-agent/reference-guide/how-it-works/developing-on-forest-admin/forest-cli-commands/set-origin)

<Info>
As your company grows, so does your development flow: you may want a more complex architecture where you have more than one layer of test environments before production (i.e preprod). This is definitely possible using Forest CLI and the right environment settings, see [here](/legacy/ruby-agent/reference-guide/how-it-works/environments#change-environment-origin).
</Info>
