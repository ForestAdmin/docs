...
To dynamically change a property within a `load` or `change` [hook](/legacy/ruby-agent/reference-guide/actions/create-and-manage-smart-actions/use-a-smart-action-form#making-a-form-dynamic-with-hooks), just set it! For instance, setting a new _description_ for the field `city`:
