...
<Info>
You may have to [add CORS headers](/legacy/ruby-agent/how-tos/setup/configuring-cors-headers) to enable the domain `app.forestadmin.com` to trigger API call on your Application URL, which is on a different domain name (e.g. _localhost:3000_).
</Info>
...
Critical actions for your business may need approval before being processed. You can require approval per role from the _Roles_ tab of your Project Settings; approval requests are then reviewed from the Collaboration menu.
