...
It also provides the Forest Admin servers with the information needed to build the User Interface (the **Forest Admin Schema**). This information includes table names, column names and types, and relationships. It is sent when you run your Admin Backend [within a file called `forestadmin-schema.json`](/legacy/javascript-agents/reference-guide/models/overview#the-forestadmin-schema-json-file).
...
The [IP whitelisting](/legacy/javascript-agents/how-tos/setup/forest-admin-ip-white-listing-forest-cloud) feature allows you to create a list of trusted IP addresses or IP ranges from which your admin users can both access to the **Forest Admin UI** and interact with your **Admin Backend**.
