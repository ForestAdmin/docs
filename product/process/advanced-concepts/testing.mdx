...
<Note>
  This keeps the check inside your Node.js test suite, without booting the full app or reaching Forest servers. For the universal method (boot the agent, then `git diff` the schema) and the Ruby v2 rake task, see [Catch an uncommitted schema in CI](/reference/schema/forestadmin-schema#catch-an-uncommitted-schema-in-ci).
</Note>
