...
Out of production (`isProduction: false`), the agent **regenerates the schema and typings** to the paths you configure when it boots. Booting also makes the agent call Forest (schema hash check, event subscription), so you must start it against a [`createForestServerSandbox`](#quick-start), a local mock that stands in for Forest's servers, with `forestServerUrl` pointing at it. Without the sandbox, `start()` would reach the real API (the default `forestServerUrl`) and either fail or push the schema to your real project. Point `schemaPath`/`typingsPath` at a temporary location, then compare what the agent generates with the committed files. Run this **only in CI**: it validates the committed state, which is irrelevant locally where the files are regenerated on every boot.
...
<Note>
  This is the Node.js approach: it boots the agent against the sandbox, so the check stays isolated from your real Forest project. Ruby v2 can instead regenerate the file directly with a rake task, no sandbox needed. See [Catch an uncommitted schema in CI](/reference/schema/forestadmin-schema#catch-an-uncommitted-schema-in-ci).
</Note>
