...
The agent only regenerates [`.forestadmin-schema.json`](/reference/schema/forestadmin-schema) in development, production uses the committed file. A developer who changes a data source or customization but forgets to commit the regenerated schema (and typings) ships a stale schema, and the change silently never reaches the UI.
