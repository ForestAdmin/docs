...
<Note>
  In Ruby, regenerate with the `rails forest_admin:schema:generate` rake task and compare with `git diff --exit-code` instead. See [Catch an uncommitted schema in CI](/reference/schema/forestadmin-schema#catch-an-uncommitted-schema-in-ci).
</Note>
