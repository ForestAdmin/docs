...
<Note>Both v1 Node.js packages get the route from their shared `forest-express` dependency, which they pin exactly — those minimums are the ones that pull in `forest-express` 10.7.0. If you audit a lockfile, that's the version to look for; `forest-express` isn't installed or configured on its own.</Note>
