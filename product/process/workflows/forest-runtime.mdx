...
<Warning>
  **Upgrade Forest Runtime before you enable the MCP trigger.** A runtime older than the release that added MCP triggering rejects MCP-started runs when it validates them: the run is created, picked up, and immediately marked failed with a validation error on its first step — so every MCP trigger in the environment fails, and `getWorkflowRun` reports the error rather than a run in progress. Nothing warns you up front; the version check happens inside the runtime. Upgrade first, then turn the toggle on.
</Warning>
