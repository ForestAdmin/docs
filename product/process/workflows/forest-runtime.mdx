...
- **The executor package**, installed at the **exact version your agent pins**. It's an *optional peer dependency* the agent loads dynamically at runtime, so `tsc` won't flag it when it's missing — but `agent.start()` throws _"The embedded workflow executor requires the `@forestadmin/workflow-executor` package"_. The pin is exact (a `^` range conflicts), so install that specific version — find it in your lockfile or with `npm info @forestadmin/agent peerDependencies`:
  ...
  ```
- **TypeScript 5.5+** — the package bundles zod 4, which requires it. TypeScript 4.x can't even parse zod 4's typings, and `skipLibCheck` won't help (it suppresses type errors, not the syntax errors these typings trigger).
...
createAgent(options)
  .addDataSource(/* ... */)
  .addWorkflowExecutor({
    database: { uri: process.env.DATABASE_URL, schema: 'forest_workflows' },
  })
  .start();
```

<Warning>
  Give the executor a dedicated `schema` (as above). If it shares the database your agent introspects and you leave `schema` unset, its `workflow_step_executions` table is created in `public` — where your SQL datasource picks it up, so it **shows up as a collection in your panel**. The executor creates the schema itself (`CREATE SCHEMA IF NOT EXISTS`), so a dedicated one just needs a name.
</Warning>
