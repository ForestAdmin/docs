...
Bedrock takes **no** `AI_API_KEY`: credentials come from the standard AWS chain — the IAM role attached to your ECS/EKS task, `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`, or a shared profile. The role needs `bedrock:InvokeModel` on the models you use. Setting `AI_API_KEY` alongside `bedrock` fails at startup rather than being ignored.
