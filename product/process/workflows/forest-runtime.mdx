...
| `ai` | Bring your own LLM instead of Forest's AI server: `{ provider: 'anthropic' \| 'openai', model, apiKey }`, or `{ provider: 'bedrock', model, region }` — Bedrock takes no `apiKey` and reads the [AWS credential chain](#amazon-bedrock), with `region` falling back to `AWS_REGION` / `AWS_DEFAULT_REGION`. Omit to keep using Forest's server. |
...
To keep AI calls off Forest's server and use your **own provider** instead, set these variables together:

| Variable | Description |
| --- | --- |
| `AI_PROVIDER` | `anthropic`, `openai` or `bedrock`. |
| `AI_MODEL` | Model name for that provider (e.g. `gpt-4.1`, `claude-sonnet-5`, `us.anthropic.claude-sonnet-4-6-v1:0`). |
| `AI_API_KEY` | Your API key for that provider. Not used with `bedrock`. |

<Note>
  This is **all-or-nothing**: set them together to use your own provider, or leave them all unset to fall back to Forest's AI server. Setting only some of them fails at startup.
</Note>

### Amazon Bedrock

With `AI_PROVIDER=bedrock`, Forest Runtime reaches the model through Amazon Bedrock, so the calls stay inside your AWS account. Set `AI_MODEL` to a Bedrock model id or inference profile id, and `AWS_REGION` to the region the model is enabled in.

**Supported models: Anthropic Claude Sonnet, Haiku and Opus.** Other Bedrock models are refused at startup. Every AI step forces a tool call, and the rest of the catalogue is either unable to honour that or unverified on our side, so an unsupported id fails fast rather than mid-run.
...
Bedrock takes **no** `AI_API_KEY`: credentials come from the standard AWS chain, resolved by the AWS SDK itself. Whatever the chain accepts, Forest Runtime accepts. The role needs `bedrock:InvokeModel` on the models you use, and setting `AI_API_KEY` alongside `bedrock` fails at startup rather than being ignored.
...
| --- | --- |
| Task role (ECS/EKS/EC2) | Nothing — the chain picks it up. |
| Role assumed from a profile | Mount `~/.aws/config` with `role_arn` + `source_profile`, and set `AWS_PROFILE`. |
