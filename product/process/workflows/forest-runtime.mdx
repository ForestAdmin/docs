...
| Option | Description |
| --- | --- |
| `database` | Postgres connection — a URI or Sequelize options. Persists run state. Required unless `inMemory`. |
...
<Note>
  These options are the complete embedded configuration. AI steps use Forest's AI server, and everything else runs on built-in defaults. To use **your own AI provider**, encrypt OAuth-protected MCP credentials, or tune the other settings (the environment variables in the sections below, including [Tuning](#tuning)), run Forest Runtime **standalone**.
</Note>
