...
**Webhook-triggered workflows require it especially.** A webhook can fire at any time, with no guarantee anyone has Forest open in a browser — so these workflows can't run browser-side. Forest Runtime executes them server-side, whenever they're triggered.
