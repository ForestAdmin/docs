...
<Note>The Python agents (`agent-python`, `django-forestadmin`) don't support Forest Runtime yet.</Note>
...
**2. Run Forest Runtime** as a Docker image or via the CLI:
...
<Note>
  The image was previously published as `ghcr.io/forestadmin/workflow-executor`. That name is still published, so existing deployments keep working — new ones should use `ghcr.io/forestadmin/runtime`. The npm package name is unchanged.
</Note>
