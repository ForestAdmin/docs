...
<Note>
  These options are the complete embedded surface. AI steps always use **Forest's AI server** — embedded can't target your own provider — and the [Tuning](#tuning) knobs (chain depth, schema cache, step and shutdown timeouts) run on their defaults; run **standalone** to change those.
</Note>
