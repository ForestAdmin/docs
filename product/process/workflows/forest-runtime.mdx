...
<Note>
  These six options are the complete embedded surface. AI steps always use **Forest's AI server** — embedded can't target your own provider — and the [Tuning](#tuning) knobs (chain depth, schema cache, step and shutdown timeouts) run on their defaults; run **standalone** to change those. Two env-based settings still apply embedded: set `FOREST_EXECUTOR_ENCRYPTION_KEY` in the agent's environment for [OAuth-protected MCP connectors](#oauth-protected-mcp-connectors), and pass a Postgres `schema` inside `database`.
</Note>
