...
<Warning>
  **Upgrade Forest Runtime before you enable the MCP trigger.** A runtime older than the release that added MCP triggering rejects MCP-started runs when it validates them: the run is created, never picked up, and the assistant sees it sit in `pending` forever. Nothing warns you — the version check happens inside the runtime. Upgrade first, then turn the toggle on.
</Warning>
