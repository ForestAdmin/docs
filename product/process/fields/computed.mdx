...
Forest Admin allows creating new fields on any collection, either computationally, by fetching data on an external API, or based on other data available on the connected data sources.

By default, the fields that you create will be read-only, but you can make them [filterable](/product/process/fields/filter), [sortable](/product/process/fields/sort), and [writable](/product/process/fields/write) by using the relevant methods.
