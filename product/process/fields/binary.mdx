...
In Forest Admin, binary fields are included in the payloads that transit between your agent and the UI like any other field.
...
To handle both cases, Forest Admin has two distinct modes available.
...
If the automatic detection based on the field type is not working for you, you can force the `datauri` mode using the `replaceFieldBinaryMode` method:
