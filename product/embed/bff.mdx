...
| `FOREST_SERVER_URL` | No | `https://api.forestadmin.com` | Forest API base URL. Set it only for another region or a local stack. |
| `FOREST_APP_URL` | No | `https://app.forestadmin.com` | Forest front base URL, used to build the sign-in redirect. Set it only for another region or a local stack. |
