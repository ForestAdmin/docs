...
| `/agent/v1/permissions` | `GET` | display hints, as described above |
...
`POST /agent/v1/ai/query` is the one exception, because it relays the Forest server. Its own refusals follow the contract, and a non-JSON or `5xx` answer from upstream keeps that status but has its body replaced with a typed `upstream_error`. A JSON answer below `500`, however, is relayed verbatim with the upstream status and the upstream's own shape — a validation error arrives as `{ errors: [{ detail }] }`, with no `error.type` to branch on. Handle that route's `4xx` bodies as the Forest server's, not the BFF's.
