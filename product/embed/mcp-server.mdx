...
upload endpoint at `<your-back-end-url>/mcp/uploads` — that host is the one to get allowed in the
next section. Objects are lost on restart, and it is correct for a **single back-end instance
...
- The in-memory store holds 64 MiB across all pending uploads (`fileUploads.ephemeralMaxTotalBytes`).
  ...
  about **three max-size files per 45-minute window**, not a rolling 64 MiB. Past that, uploads are
  refused with a `507`.
