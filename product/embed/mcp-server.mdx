...
<Warning>
  When mounted, the MCP server registers its OAuth and `.well-known` routes at your back-end's root, including `/oauth/authorize`, `/oauth/token` and `/.well-known/*`. **If your back-end already serves its own OAuth or `.well-known` routes, they will be shadowed and break.**

  Pass a `basePath` to move every MCP route under a dedicated prefix so both coexist:
