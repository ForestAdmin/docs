  ...
  "workflowHistory": [
    {
      ...
      "stepIndex": 0,
      "done": true,
      "context": { "manuallyCompleted": true, "selectedOption": "Approve" },
      "stepDefinition": {
        "type": "task",
        "taskType": "guideline",
        "title": "Review the KYB documents",
...
In this first version the assistant can *observe* a parked run but not answer it. When the run sits on a human step — an incomplete step (`done: false`) whose `stepDefinition.executionType` is `manual` — it is routed to the workflow's **fallback inbox** (when one is configured), and someone finishes it from the Forest UI. Relaying the step's question into the chat and submitting the answer through MCP is not available in this version.
...
- **Turning it off** — disable a single workflow's MCP toggle, or set an [`enabledTools`](#restrict-tools) allowlist that leaves `triggerWorkflow` out to remove MCP triggering across all workflows (when `enabledTools` is unset, every tool is enabled — restricting means listing the tools you keep). Either way, manual and webhook starts of that workflow keep working. Revoking the connected user's access is the lever that also halts runs already in flight — they are not aborted, but their next data step fails once the permissions are gone — see [Revoking MCP access](/product/process/workflows/triggers#revoking-mcp-access).
