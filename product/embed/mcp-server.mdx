...
| `workflowHistory` | The ordered steps the run has reached. Each entry pairs the step's resolved **definition** (`stepDefinition`: `type`, `title`, an optional `taskType` and `prompt`, and the `outgoing` branches) with its **outcome** (`done`, plus a `context` carrying the selected option, a manual completion, an escalation state, an awaiting-input reason, or an `error`). The last entry with `done: false` is where the run currently sits. |

Identifying fields — `id`, `workflowId`, `collectionId`, `selectedRecordId`, timestamps — are returned too.
