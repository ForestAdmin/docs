...
In this first version the assistant can *observe* a parked run but not answer it. When the run sits on a step that needs a person — an incomplete step (`done: false`) whose `stepDefinition.executionType` is `manual` or `automated-with-confirmation` (automated, but awaiting a human confirmation) — it is routed to the workflow's **fallback inbox** (when one is configured), and someone finishes it from the Forest UI. Relaying the step's question into the chat and submitting the answer through MCP is not available in this version.
