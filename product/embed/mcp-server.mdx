...
- **Four tools are not audited** — `listWorkflows`, `getWorkflowRun`, `getActionForm` and `requestActionFileUpload` leave no Activity Logs entry. For the two workflow reads the log route needs a resource they have no way to name; `getActionForm` and `requestActionFileUpload` predate this and are unaudited for historical reasons. `requestActionFileUpload` is worth calling out separately: it is not a read — it mints a pre-authorized upload, and it is enabled by default whenever file uploads are configured. Every other tool, read or write, writes an entry. Closing these gaps is tracked in PRD-967.
...
- Logs every write, and every read except four tools, for audit purposes (see [Identity, auditing, and limits](#identity-auditing-and-limits))
