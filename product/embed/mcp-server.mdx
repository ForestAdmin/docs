...
upload endpoint at `<your-agent-url>/mcp/uploads` — that host is the one to get allowed in the
...
<Warning>
  **On a deployed standalone server, configure a storage backend.** The in-memory store builds its
  upload URLs from `http://localhost:<port>`, which is what the standalone server knows about
  itself — so remote clients receive URLs pointing at their own machine. With a storage backend the
  URL comes from that backend instead and the problem disappears. Mounted deployments are
  unaffected: their upload URLs derive from the back-end URL registered in Forest.
</Warning>
