...
Every piece of data in Forest Admin is represented by a **field**. Every field is rendered using a **widget**. Understanding this distinction helps you configure exactly how your data looks and behaves for operators.
...
- **Date Range Picker** — two date pickers for selecting a start and end date. Best used in filters and Smart Action forms.
...
**Edit widgets:**
- **Dropdown** — a select menu with predefined options. Options can be static (defined manually) or dynamic (loaded from another collection via a Smart Action).
...
- **Search & Select** — an autocomplete input that searches across a related collection as the operator types. More powerful for large related collections.
