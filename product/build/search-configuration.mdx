...
Forest Admin includes a free-text search bar on every collection's table view. By default it searches across text, enum, number, and UUID fields. You can configure exactly which fields are searched, what operators are used, and even replace the default behavior entirely with custom logic.

## How Search Works

When an operator types in the search bar, Forest Admin sends a query to your agent with the search string. The agent applies it as a filter against your data source and returns matching records.
...
By default, Forest Admin searches only specific field types:
...
<CardGroup cols={2}>
  <Card title="Collection Configuration" icon="sliders" href="/product/build/collection-configuration">
    Overview of all collection-level configuration options
  </Card>
  <Card title="Fields & Filters" icon="filter" href="/product/build/fields-and-widgets/overview">
    Configure filterable fields and operators
  </Card>
  <Card title="Browse" icon="table" href="/product/execute/browse">
    How operators interact with search and filters
  </Card>
</CardGroup>
