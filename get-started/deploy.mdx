...
Generate `.forestadmin-schema.json` in your CI pipeline without starting the agent or contacting Forest, then bake it into your build artifact.

<Tabs>
  <Tab title="Node.js">
    Call [`agent.generateSchemaOnly()`](/reference/agent-api/nodejs) (available since `@forestadmin/agent` 1.83.0) from a one-shot script. Extract your agent setup into a shared factory that does **not** call `.start()`, so both your server and this script can import it:
  ...
  <Tab title="Ruby">
    Run the `forest_admin:schema:generate` rake task, which writes the file without booting the server or sending it to Forest:
...
Once you generate at build time, stop tracking the file in Git so a stale committed copy can't be shipped by mistake. Adding it to `.gitignore` is not enough on its own — an already-committed file stays tracked until you remove it from the index:

```bash
git rm --cached .forestadmin-schema.json
echo '.forestadmin-schema.json' >> .gitignore
git commit -m "Stop tracking .forestadmin-schema.json"
```
