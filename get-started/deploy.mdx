...
<Steps>
  <Step title="Decide where it runs">
    On Node.js, default to a separate service, so the back-end's restarts and boot work stay off the process serving your users — you can still reuse your ORM models from a shared codebase. On Ruby the back-end runs inside your application, since standalone deployment is currently available for Node.js only. See [Deployment options](/get-started/connect/architectures/deployment-options).
  </Step>
  ...
  <Step title="Deploy your back-end to your infrastructure">
    ...
    On a SQL datasource with a wide schema, consider [pre-computing the introspection](/get-started/connect/data-sources/sql#pre-compute-the-introspection) so restarts don't re-scan it.
  </Step>
...
<Warning>
This check reads your data model the same way your agent does. If you supply a [pre-computed introspection](/get-started/connect/data-sources/sql#pre-compute-the-introspection), the regenerated schema comes from that file rather than from the database, so a stale introspection produces a matching stale schema and the check passes. Regenerate the introspection on every migration to close that gap.
</Warning>
