...
<Steps>
  <Step title="Decide where it runs">
    On Node.js, run it as its own service by default. On Ruby, it runs inside your application. See [Deployment options](/get-started/connect/architectures/deployment-options).
  </Step>
...
<Warning>
With a [pre-computed introspection](/get-started/connect/data-sources/sql#pre-compute-the-introspection), this check reads that file instead of the database, so it can't catch a stale one. Regenerate the introspection on every migration.
</Warning>
