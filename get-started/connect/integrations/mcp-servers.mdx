...
Workflow steps run on [Forest Runtime](/product/process/workflows/forest-runtime), which needs to renew a connector's access on its own. An OAuth connector therefore works only if the provider issues a **refresh token**. Some providers issue one only when a setting is turned on in your OAuth app, so check the provider's documentation before you connect. Without a refresh token, the connector is created but can't be used, and connecting again doesn't help until the setting is on.
