...
<Note>
  The exact operations available come from each vendor's MCP server and can change without notice — that's why Forest doesn't duplicate the full tool list here. After connecting, the **"Confirm the exposed tools"** step shows you exactly what's available for your account and plan.
</Note>
