...
On a small schema this is invisible. On a large one it is a measurable spike in database load and event-loop time on every boot, restart and redeploy — which is why [pre-computing the introspection](#pre-compute-the-introspection) is the recommended production setup, and why the agent belongs in [its own process](/get-started/connect/architectures/deployment-options) when it sits next to an application serving live traffic.
