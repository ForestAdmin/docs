...
Pylon has no way to read the threads of several issues at once, so messages are not exposed as their own collection. Instead, each issue carries a structured `messages` array column, fetched from `GET /issues/{id}/messages` only when the projection asks for it — in practice, when `messages` is rendered on the detail view. A custom action cannot read the thread: `ActionContext#get_records` lists with an empty projection whatever fields it is handed, so `context.get_record(['messages'])` comes back without the column.
...
- **Custom fields** are filtered through their slug, with the operators of the column the introspection built: the equality and presence families, plus the whole substring set on a `String` — negations included — and the bare comparisons on a date, which drops `IN` / `NOT_IN` in exchange (the toolkit's `Rules` grants a `Date` or a `Dateonly` column no array operator). A `Number` gets no comparison (Pylon documents `time_is_after` / `time_is_before` and nothing else, so a numeric range would travel as a time filter), and a `multiselect` gets nothing.
- **Presence filters need a field that supports them.** `PRESENT` / `BLANK` / `MISSING` are advertised on every field carrying an equality or a membership filter: the agent derives them above the datasource and rewrites them into a comparison with an empty value. A field the tables above list them on answers through `is_set` / `is_unset` — the one way Pylon matches an absent value — and never through that rewritten comparison. On every other field the translator refuses it and names the filter to change, rather than sending a comparison Pylon would answer as if the empty value were a value of its own.
...
- **`NOT_EQUAL` is offered wherever `NOT_IN` is**, although no table above lists it: the datasource declares the one spelling Pylon takes, and the agent republishes the other from it. The same mechanism puts `PRESENT` / `BLANK` / `MISSING` in front of every field carrying an equality or a membership filter — the ones that cannot answer them included, which is what the bullet above is about.
...
That fan-out is capped **per page** at `MAX_ID_LOOKUPS` (**20**): the window is taken off the ids before any of them is read, so a paginated read walks a wider selection a page at a time.

A filter carrying **no page** has no window to take, though — what an action's own read goes through, and any API caller that sends none — and it is truncated at its first twenty ids, with a warning in the log. The one shape refused rather than truncated is a lookup carrying a residual condition: which records the page holds could only be known by reading all of them, so past the cap it is refused outright.
...
`PylonAccount` and `PylonContact` need none of this capping: their search endpoints filter `id` server-side, so a selection of any width costs one request. A record detail — `id equals X` alone — is still read through `GET /accounts/{id}` / `GET /contacts/{id}`, to spend the cheaper budget rather than because the search could not answer it.
...
Forest's offset/limit window is translated into Pylon's cursor pagination: the client walks the cursor until it has collected the window the caller asked for, capped at `MAX_PAGES` (**20**) pages and `MAX_RECORDS` (**5000**) records per walk, with a page size clamped to `MAX_SEARCH_LIMIT` (**1000**). A walk cut short by those caps logs a truncation warning.
...
Retries apply to `429`, `502`, `503` and `504`, plus timeouts and dropped connections. Only `GET`, `HEAD` and `OPTIONS` are replayed, and that restriction covers the `5xx` as much as the transport failures — a `502`, or a dropped connection, on the way back from a `DELETE` Pylon did perform would otherwise be replayed into a 404 and reported as a deletion that failed when it landed. A `502` answering `POST /issues/search` is therefore not replayed either. A `429` is retried on any verb, Pylon having rejected the request before processing it.

## Errors

Every class below lives under `ForestAdminDatasourcePylon::`. The table drops the prefix for width, but a `rescue` needs it in full — `rescue ForestAdminDatasourcePylon::APIError`.
