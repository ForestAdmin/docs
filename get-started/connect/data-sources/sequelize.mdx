...
Forest writes through `bulkCreate`, `update` and `destroy` without `individualHooks`, so only bulk hooks fire:

| Operation in Forest | Hooks that run | Hooks that do **not** run | Validations |
| --- | --- | --- | --- |
| Create a record | `beforeBulkCreate`, `afterBulkCreate` | `beforeCreate`, `afterCreate` | Skipped |
...
<Warning>
A per-record hook that enforces an invariant, such as writing an audit row, won't fire on a Forest edit. Move it into an [agent hook](/product/process/advanced-concepts/hooks/overview).
</Warning>

<Note>
The [SQL datasource](/get-started/connect/data-sources/sql) never loads your models, so none of your hooks or validations apply to it.
</Note>
