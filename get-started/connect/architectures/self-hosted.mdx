...
| **Forest Admin UI** | Forest Admin | `app.forestadmin.com` | • Back-office interface for your users<br/>• Communicates with your agent via HTTPS<br/>• Receives only schema metadata (no data)<br/>• Managed and updated by Forest Admin |
...
Standalone deployment runs the Forest Admin agent as a completely separate service. It operates in its own process or container with dedicated resources and its own database connection pool. Your application and the back-office are deployed and scaled independently.
