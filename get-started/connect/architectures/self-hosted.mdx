...
This approach is valuable when you want to leverage your existing ORM models. If you use Sequelize, Mongoose, ActiveRecord or Mongoid, Forest reads your model definitions directly, with no need to redefine your schema. You can also reuse your existing business logic, sharing code, services, and utilities between your application and Forest without duplication.
...
<Warning>
A shared process is also a shared restart schedule, event loop and crash surface with the application serving your users. Read [Choosing a deployment topology](/get-started/connect/architectures/deployment-topology) before committing to in-app.
</Warning>
...
<Info>
**Recommendation**: We recommend **standalone deployment** for cleaner architecture and better separation of concerns. Use **in-app** only if you need to reuse your ORM models or existing business logic — see [Choosing a deployment topology](/get-started/connect/architectures/deployment-topology) for how to make that call, and how to stay standalone inside a monorepo.
</Info>
