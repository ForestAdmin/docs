...
This approach is valuable when Forest needs your application's running code: an in-process service, a transaction to join, a singleton your business logic depends on. If you use Sequelize, Mongoose, ActiveRecord, Mongoid, Django or SQLAlchemy, Forest also reads your model definitions directly — though that alone does not require in-app, since importable models work from a separate process too.

If your ORM has no matching datasource — TypeORM, Prisma, Drizzle and others — Forest introspects the database instead and cannot read your entities. In-app then gains you nothing on model reuse while still sharing the process.

<Warning>
A shared process is also a shared restart schedule, event loop and crash surface with the application serving your users. Read [Deployment options](/get-started/connect/architectures/deployment-options) before committing to in-app.
</Warning>
...
<Info>
**Recommendation**: We recommend **standalone deployment** for cleaner architecture and better separation of concerns. Reusing your ORM models does not require in-app — importing them from a shared codebase works from a separate process. See [Deployment options](/get-started/connect/architectures/deployment-options) for what genuinely calls for in-app, and how to stay standalone inside a monorepo.
</Info>
