...
Single Sign-On (SSO) allows users to access Forest using their existing corporate credentials through your identity provider (IdP). This centralizes authentication, improves security, and provides a better user experience.
...
<CardGroup cols={2}>
  <Card title="Centralized Authentication" icon="key">
    Users authenticate once through your IdP and access Forest
  </Card>
  ...
  <Card title="Simplified Onboarding" icon="user-plus">
    New employees automatically get Forest access through your IdP provisioning
  </Card>

  <Card title="Instant Offboarding" icon="user-minus">
    Disabling a user in your IdP immediately revokes Forest access
  </Card>
</CardGroup>

## Supported identity providers

Forest supports any SAML 2.0-compliant identity provider:
...
<CardGroup cols={2}>
  ...
  <Card title="AWS SSO" icon="aws" href="/get-started/control/authentication/sso-providers/aws">
  ...
  <Card title="Generic SAML" icon="diagram-project" href="/get-started/control/authentication/sso-providers/generic-saml">
...
SSO with Forest uses the SAML 2.0 protocol:
...
<Steps>
  <Step title="User Access">
    User navigates to your Forest URL
  </Step>

  <Step title="SAML Request">
    Forest generates a SAML authentication request and redirects to your IdP
  </Step>
  ...
  <Step title="SAML Response">
    IdP sends a signed SAML assertion back to Forest with user attributes
  </Step>

  <Step title="Session Creation">
    Forest validates the SAML assertion and creates a session
  </Step>

  <Step title="Access Granted">
    User is logged in and can access Forest based on their role
  </Step>
</Steps>
...
<Steps>
  <Step title="Enable SSO in Forest">
  ...
  <Step title="Configure Your Identity Provider">
    ...
    - **ACS URL**: Copy from Forest settings
    - **Entity ID**: Copy from Forest settings
  ...
  <Step title="Configure User Provisioning">
    ...
    - **SCIM Provisioning**: Sync users automatically (see [SCIM documentation](/get-started/control/authentication/scim))
  </Step>
...
SAML assertions contain user information that Forest maps to user profiles:

### Required attributes

| SAML Attribute | Forest Field | Description |
|----------------|-------------------|-------------|
| `email` or `NameID` | Email | User's email address (must match) |

### Optional attributes

| SAML Attribute | Forest Field | Use Case |
|----------------|-------------------|----------|
...
<Steps>
  <Step title="Enable JIT Provisioning">
    In Forest SSO settings, enable **Automatic User Creation**
  </Step>
...
4. Forest matches by email and grants access

### SCIM provisioning

Automatically sync users from your IdP. See [SCIM Documentation](/get-started/control/authentication/scim) for setup.

## Enforcing SSO

Once SSO is configured and tested, enforce SSO-only authentication:

<Warning>
**Emergency Access**: Always keep at least one admin account with password authentication enabled. This ensures access to Forest if SSO has issues.
</Warning>
...
<AccordionGroup>
  <Accordion title="Error: Invalid SAML Response">
    **Possible Causes**:
    - Clock skew between Forest and IdP (check time sync)
    - Invalid signature (check certificate)
    - Wrong ACS URL configured in IdP

    **Solution**:
    1. Verify ACS URL in IdP matches Forest
    2. Ensure IdP certificate is valid and not expired
    3. Check IdP and Forest server times are synchronized
  </Accordion>

  <Accordion title="Error: User Email Not Found">
    **Possible Causes**:
    - Email in SAML assertion doesn't match Forest user
...
**Solution**:
1. Ensure Forest has the correct IdP metadata
...
<CardGroup cols={2}>
  ...
  <Card title="Forest Logs" icon="list">
    Review Forest audit logs for SSO authentication events
  </Card>
</CardGroup>
...
1. **Limit IdP Access**: Only authorize users who need Forest access
2. **Use Groups**: Assign Forest access to IdP groups, not individual users
...
- **Track Login Events**: Monitor SSO logins in [audit logs](/get-started/control/audit)
- **Alert on Failures**: Set up alerts for failed SSO attempts
- **Review Access**: Regularly review who has accessed Forest via SSO
...
<Tabs>
  <Tab title="SP-Initiated (Recommended)">
    **User starts at Forest**

    1. User goes to Forest URL
    2. Clicks "Log in with SSO"
    3. Redirected to IdP
    4. Logs in and returns to Forest
  ...
  <Tab title="IdP-Initiated">
    **User starts at IdP portal**
    ...
    2. Clicks Forest app icon
    3. Directly lands in Forest
...
<CardGroup cols={2}>
  <Card title="SCIM Provisioning" icon="users-gear" href="/get-started/control/authentication/scim">
    Automate user provisioning from your IdP
  </Card>

  <Card title="2FA Setup" icon="mobile-screen" href="/get-started/control/authentication/2fa">
    Add additional security with two-factor authentication
  </Card>

  <Card title="Teams & Roles" icon="user-group" href="/get-started/control/teams">
    Organize SSO users into teams with appropriate roles
  </Card>

  <Card title="Audit Logs" icon="clock-rotate-left" href="/get-started/control/audit">
