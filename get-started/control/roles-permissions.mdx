...
<Warning>
All permissions of the destination environment are overridden, for **all roles**.
</Warning>

The copy runs in the background and can take a few minutes. You receive a notification and an email once it's done, then refresh your browser to see the updated permissions.

Requirements:
- Only admins can copy permissions
- Your project must have at least two remote environments (production included). Development environments are not eligible
- Both environments must have the same schema, otherwise the copy fails
