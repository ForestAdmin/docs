...
- **A record-level scope only protects a record that still exists.** A scope can't be evaluated against a record that is gone, so once it is genuinely deleted, anyone with read permission on the collection can see that something happened to it, by whom and when — seeing what was deleted is much of the point of an audit trail. Both agents stop short of the actual values on the record's history route: a `create` or `delete` row's captured values are withheld if they themselves would have failed the caller's scope. They differ on the rest. Node.js withholds an `update` row's values unconditionally in that situation, a partial diff being unreliable to test. The Ruby agent tests each side of an update against its own values, and evaluates a scope only against a snapshot that can answer it — a read-only column it never captured, a redacted value, or a `pending` row's unproven id all read as unanswered, which withholds.
