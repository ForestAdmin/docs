---
title: "Audit Trail"
description: "Record who changed what, with before/after values, and read it back from the record's Activity tab"
---

Activity logs tell you **who did what, and when** — without the data. The **audit trail** adds the missing half: for every create, update and delete Forest performs through your agent, it records the **field-level before/after values** into a SQL database you own, and the record's **Activity** tab replays them.

It has two halves:

- **Capture & storage** — built into the agent. It instruments every collection that has a primary key through the collection hooks (so it is datasource-agnostic: SQL, Sequelize, Mongo, ActiveRecord, Mongoid…) and writes one row per changed record into a SQL database you provide. A collection without one is skipped — there is nothing to file its rows under, and Node.js logs a warning naming it.
- **The Activity tab** — in Forest, on each record's detail page. It replaces the previous activity tab: it lists the record's activity logs and enriches each one with the field-level diff captured by your agent.
...
<Tabs>
  ...
  <Tab title="Ruby">
    ...
    Storage uses ActiveRecord. Outside Rails, add `gem 'activerecord'` and your adapter gem to the Gemfile — nothing is loaded and no connection is opened while the feature is unconfigured. Once it is configured, the connection is opened and the pending migrations applied during agent startup, so an unreachable database or a failed migration fails the boot rather than the first audited write.
  </Tab>
</Tabs>
...
A write that matches a record but changes nothing leaves no row `pending` either way — the write itself resolved. On Node.js its row is confirmed to `done` with `previous_values`/`new_values` both empty; on Ruby the pending row is deleted, so a no-op write leaves no entry at all.

### Correlation with activity logs

The agent generates one id per request, exposes it to the client in the `X-Forest-Correlation-Id` response header, and stores it as `correlation_key` on every row written during that request. That id is what lets Forest join an activity log ("who triggered what") to the audit rows ("which fields changed"). The Node.js agent and the Rails integration set up the header and its CORS exposure for you. A standalone Ruby agent does not: mount `ForestAdminAgent::Http::CorrelationIdMiddleware` yourself, otherwise the id is missing from responses and a pooled thread hands its previous request's key to the next one.
...
Each record's detail page keeps a single **Activity** tab — the new one replaces the previous activity tab — showing a chronological, paginated timeline of every change made to that record.
...
The tab is present on every record, whether or not the audit trail is on. Field-level diffs only appear for agents that have the audit trail configured; otherwise the timeline shows activity logs alone.
...
When the audit trail is configured, the agent exposes these routes. They sit behind Forest's authentication and require read permission on the target collection. The Activity tab is their consumer; you can also query them directly.
...
| `GET /forest/_audit-trail/correlation/{correlationKey}?collection=…&recordId=…` | `{ data }` — the operations recorded under one correlation key |
| `GET`/`POST` `/forest/_audit-trail/correlations` | `{ data }` — a flat list for several correlation keys at once (`correlationKeys` comma-separated on `GET`, a JSON array on `POST`) |

Both correlation routes are scoped to a single record: they require `collection` and `recordId` (query params on `GET`, body on `POST`) and answer `400` without them.
...
| `fields` | comma-separated field names | keep entries whose diff touched one of them — see the adapter note below |
| `search` | free text, trimmed; empty ⇒ ignored | keep entries matching the term — see below |

<Warning>
**`fields` and `search` depend on the audit database's adapter.** Both interrogate the stored JSON, which every engine spells differently, so an unsupported one makes the request **fail** rather than fall back to an unfiltered answer. `fields` works on PostgreSQL, SQLite and MySQL/MariaDB on both agents, plus SQL Server on Node.js (2022+, for `JSON_PATH_EXISTS`). `search` works on any dialect on Node.js, and on PostgreSQL, SQLite and MySQL/MariaDB on Ruby. Anywhere else, the two filters raise — the rest of the route, including the unfiltered history, is unaffected.
</Warning>
...
- **A record-level scope only protects a record that still exists.** A scope can't be evaluated against a record that is gone, so once it is genuinely deleted, anyone with read permission on the collection can see that something happened to it, by whom and when — seeing what was deleted is much of the point of an audit trail. The Node.js agent, and the Ruby agent from **1.44.1**, stop short of the actual values on the record's history route: a `create` or `delete` row's captured values are withheld if they themselves would have failed the caller's scope. They differ on the rest. Node.js withholds an `update` row's values unconditionally in that situation, a partial diff being unreliable to test. The Ruby agent tests each side of an update against its own values, and evaluates a scope only against a snapshot that can answer it — a read-only column it never captured, or a value stored redacted, reads as unanswered, which withholds. **A Ruby agent between 1.39.0, where the trail shipped, and 1.44.0 does not filter at all**: for a deleted record it returns the recorded `previousValues`/`newValues` to anyone with read permission on the collection, scope or not.

  **The withholding covers that one route.** `/state` reconstructs the same record from the same rows, and the correlation routes check the record but not the values, so on both agents a caller the history route withholds from can still read those values one request away.
