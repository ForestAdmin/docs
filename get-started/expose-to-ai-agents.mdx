...
Every operation on data runs with the permissions of the authenticated Forest user, and is logged just like a UI action — with four exceptions, listed under [Identity, auditing, and limits](/product/embed/mcp-server#identity-auditing-and-limits). Starting a workflow is the one exception to the permission half: it is gated by the user's rendering and the workflow's MCP toggle only, and neither the target record nor the workflow's segments are checked when the run is created. What the run then reads and writes is bound by that user's permissions as usual.
