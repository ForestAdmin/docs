---
title: Forest Admin documentation
description: Build powerful admin panels on top of your data
---

# Forest Admin documentation

Build powerful admin panels on top of your data

## Quick start

Choose your deployment option:

* [Cloud](/get-started/quickstart-cloud.md) - Get started in 60 seconds
* [Self-Hosted](/get-started/quickstart-self-hosted.md) - Full control over your data
* [On-Premise](/get-started/quickstart-on-premise.md) - Maximum security

## Explore

* [Custom Actions](/product/process/actions/custom-actions/overview.md) - Build custom operations
* [Data Sources](/product/integration/data-sources/overview.md) - Connect your data
* [Roles & Permissions](/product/control/roles-permissions.md) - Access control

## Learning paths

* [Developer Guide](/guides/learning-paths/developer.md)
* [Operations Guide](/guides/learning-paths/operations.md)
* [Business Guide](/guides/learning-paths/business.md)

## Resources

* [API Reference](/reference/agent-api/nodejs.md)
* [Migration Guide](/guides/migration/from-v1/overview.md)
* [Community Forum](https://community.forestadmin.com)
* [GitHub](https://github.com/ForestAdmin)
