...
This makes the endpoint safe to retry: a caller that retries after a network error either starts the run (`202`) or learns it was already handled (`409`).
...
| Status | Meaning |
|---|---|
| `400` | Invalid JSON body, missing `record_id`, or a malformed / tampered URL. |
