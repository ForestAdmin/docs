...
{% hint style="info" %}
We use the option `isProduction` in the `createAgent` function. {{#nodejs}}You can use `isProduction: process.env.NODE_ENV === 'production'` to control this behavior. {{/nodejs}}Setting this variable to `false` will regenerate a new `.forestadmin-schema.json` file every time your app restarts. Using `true` will never regenerate the file.
{% endhint %}
