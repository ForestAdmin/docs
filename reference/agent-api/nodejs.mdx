...
The secrets, the Forest URLs and the logger are inherited from the agent and cannot be overridden. Every option is a feature switch, so what you leave out stays off, and `GET /bff/health` reports which surfaces are on under `configured`.
