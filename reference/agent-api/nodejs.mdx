...
Configure an AI provider to power Forest Admin's AI features (natural language search, smart actions, etc.).
...
Initialize a cloud customization project. Authenticates with Forest Admin, creates a `cloud-customizer` directory, configures credentials, and generates type definitions.
...
Refresh your Forest Admin authentication token.
...
`@forestadmin/agent-testing` provides utilities to test agent customizations locally without connecting to Forest Admin servers.

### createForestServerSandbox(port)

Start a local sandbox that mimics Forest Admin servers.
