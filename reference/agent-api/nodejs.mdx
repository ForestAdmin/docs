...
<Note>
Changing `prefix` changes the URL Forest calls, so update the environment's URL in Forest to match. Two agents on one host need different prefixes — see [Running two agents at once](/get-started/connect/architectures/deployment-options#running-two-agents-at-once).
</Note>
