...
| `defaultTimezone` | `string` | — | Fallback IANA timezone for a request that carries none. Without it, such a request answers `400 missing_timezone`; embedded there is no `BFF_DEFAULT_TIMEZONE`, so this option is the only way to set the fallback. |
