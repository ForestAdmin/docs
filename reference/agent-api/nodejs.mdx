...
<Warning>
An agent answers every request under its mount path, so two agents on one host with the same `prefix` do not coexist — one of them silently takes all the traffic. Give a second agent its own hostname, or its own `prefix` **and** a matching environment URL in Forest: changing only one of the two leaves the agent unreachable or returning 404. See [Choosing a deployment topology](/get-started/connect/architectures/deployment-topology).
</Warning>
