...
<Note>
  Despite the name, `Only` means it *only generates the files* without starting the agent or sending the schema to Forest, not "the schema only". So with `typingsPath` set, a single call produces both the schema and the typings. If you want to regenerate **only** the typings, use `agent.updateTypesOnFileSystem()` (documented above) instead.
</Note>
