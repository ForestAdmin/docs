...
Run a [Forest BFF](/product/embed/bff) inside the agent process, so a trusted browser UI can call your agent over REST without a second deployment and without a second port.
...
|--------|------|---------|-------------|
| `allowedOrigins` | `string[]` | `[]` | Origins (scheme, host, port) allowed to call the BFF from a browser. Matching is exact, except for a single leading wildcard label from `@forestadmin/agent-bff` 1.31.0, `https://*.example.com`. Empty means no cross-origin access. See [Cross-origin access](/product/embed/bff#cross-origin-access). |
...
| `aiTimeoutMs` | `number` | `120000` | How long the AI relay waits for the Forest server on `POST /bff/agent/v1/ai/query`. |
...
The secrets, the Forest URLs and the logger are inherited from the agent and cannot be overridden. The feature switches default to off, so what you leave out stays off, and `GET /bff/health` reports which surfaces are on under `configured`.
