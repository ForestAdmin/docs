...
With `basePath: '/ai'`, every route moves under the prefix, leaving your application's own routes untouched:
...
| `GET /.well-known/oauth-authorization-server/ai` | Authorization server metadata |
| `GET /.well-known/oauth-protected-resource/ai/mcp` | Protected resource metadata |
