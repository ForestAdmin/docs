...
|--------|------|-------------|
| `authSecret` | string | Your `FOREST_AUTH_SECRET`. Any long random string. |
| `envSecret` | string | Your `FOREST_ENV_SECRET`, a 64-character hexadecimal string retrieved from Forest Admin. |
| `isProduction` | boolean | Production mode. When `true`, the schema is read from `schemaPath` instead of being rebuilt, typings are not regenerated, and error details and chart route URLs are no longer logged. |

#### Connection and routing

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `prefix` | string | `''` | Path the Forest routes are mounted on (e.g. `/api/v1`). Only `-`, `~`, `/`, letters, digits and `_` are allowed. |

<Warning>
An agent answers every request under its mount path, so two agents on one host with the same `prefix` do not coexist — one of them takes all the traffic, with nothing logged to say so. Give a second agent its own hostname, or its own `prefix` **and** a matching environment URL in Forest: changing only one of the two leaves the agent unreachable or returning 404. See [Deployment options](/get-started/connect/architectures/deployment-options).
</Warning>
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `schemaPath` | string | `.forestadmin-schema.json` | Path of the schema file. Written on every start outside production, read at start in production. |
| `typingsPath` | string \| null | `null` | Path of the generated TypeScript typings file. Nothing is written when unset, or when `isProduction` is `true`. |
| `typingsMaxDepth` | number | `5` | How deep relationships are introspected when generating typings. |
| `skipSchemaUpdate` | boolean | `false` | Never send the schema to Forest. Mainly used by Forest Cloud; logs a warning at start, and is incompatible with `experimental`. |
| <code style={{whiteSpace: 'nowrap'}}>ignoreMissingSchemaElementErrors</code> | boolean | `false` | Do not throw when a customization targets a collection or a field that does not exist. Useful when the same code runs against databases with different schemas. |

#### Logging and errors

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `logger` | `(level, data) => void` | `console.error` | Custom logger. Receives the level and the message or error. |
| `loggerLevel` | string | `'Info'` | Minimum level passed to the default logger: `'Debug'`, `'Info'`, `'Warn'` or `'Error'`. Ignored when you provide your own `logger`. |
| `customizeErrorMessage` | `(error: Error) => string \| null` | `null` | Rewrite the message sent to the UI for unexpected errors. Return `null` to fall back to the default message. |

#### Permissions and caching

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `instantCacheRefresh` | boolean | `true` | Keep permissions in sync through a Server-Sent Events connection to Forest. Set it to `false` when your infrastructure does not support long-lived connections; the agent then falls back to a periodic refresh. |
| <code style={{whiteSpace: 'nowrap'}}>permissionsCacheDurationInSeconds</code> | number | `900` | Permission cache lifetime when `instantCacheRefresh` is `false`. Values below `60` are raised to `60`. Ignored (with a warning) when `instantCacheRefresh` is `true`. |
| <code style={{whiteSpace: 'nowrap'}}>skipRelationReadPermissions</code> | boolean | `false` | Serve columns of collections the caller has no `read` permission on when they are reached through a relation path (e.g. `holder:nationalId`) in a projection, filter, sort, search or chart, and drop the `browse` permission a `Count` leaderboard requires on the collection it counts. The `browse`/`read`/`export` check on the queried collection itself still runs. |
| `maxRecordsForApproval` | number | `500` | Maximum number of records a "select all" approval-required action may target. Must not exceed the Forest server cap of `500`. |

<Warning>
  `skipRelationReadPermissions` widens what your users can read. The agent logs a
  warning at startup when it is enabled.
</Warning>
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `bodyParserOptions` | object | `{ jsonLimit: '50mb' }` | Forwarded to [`@koa/bodyparser`](https://github.com/koajs/bodyparser?tab=readme-ov-file#options). Accepts `jsonLimit` (number of bytes or size string) and `enableRawChecking`. |
| `maxBodySize` | string | `'50mb'` | **Deprecated**, use `bodyParserOptions.jsonLimit` instead. |
| `limitExportSize` | number | _unlimited_ | Maximum number of records a CSV export returns. |
| <code style={{whiteSpace: 'nowrap'}}>useUnsafeActionEndpoint</code> | boolean | `false` | Drop the positional index from Smart Action routes. |
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `workflowExecutorUrl` | string \| null | `null` | Base URL of the [Forest Runtime](/product/process/workflows/forest-runtime) instance workflow requests are proxied to. The workflow routes return `404` while it is unset. |
| `auditTrail` | object \| null | `null` | Record every create, update and delete into a SQL database and expose the record history routes. See [Audit trail](/get-started/control/audit-trail). |
| `experimental` | object | `null` | Opt in to no-code customizations built in the Forest UI: `webhookCustomActions` and `updateRecordCustomActions`. Incompatible with `skipSchemaUpdate`. |
| `forestAdminClient` | ForestAdminClient | _built from the options_ | Replace the client used to talk to the Forest API. Intended for tests and internal tooling. |
...
### agent.addBff(options?)

Run a [Forest BFF](/product/embed/bff) inside the agent process, so a trusted browser UI can call your agent over REST without a second deployment and without a second port. Since `@forestadmin/agent` 1.100.0 — on an earlier agent the call throws `agent.addBff is not a function`, whatever version of `@forestadmin/agent-bff` is installed.
...
Requires the `@forestadmin/agent-bff` package:
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `allowedOrigins` | `string[]` | `[]` | Origins (scheme, host, port) allowed to call the BFF from a browser. Matching is exact, except for a single leading wildcard label, `https://*.example.com`, which needs `@forestadmin/agent-bff` 1.31.0 or later. Empty means no cross-origin access. See [Cross-origin access](/product/embed/bff#cross-origin-access). |
| `tokenEncryptionKey` | `string` | — | Base64-encoded 32-byte key. Enables the sign-in routes; without it nobody can sign in from a browser UI. |
| `defaultTimezone` | `string` | — | Fallback IANA timezone for a request that carries none. Without it, such a request answers `400 missing_timezone`. Embedded there is no `BFF_DEFAULT_TIMEZONE` to fall back on, so this option is the only way to set it. |
| `agentTimeoutMs` | `number` | `10000` | How long a data or action call to the agent may take. It bounds the wait and cancels nothing: a write cut off at the deadline still commits inside the agent, so a client retrying on the timeout can double a non-idempotent mutation. |
| `aiTimeoutMs` | `number` | `120000` | How long the AI relay waits for the Forest server on `POST /bff/agent/v1/ai/query`. That route needs an OAuth session: called with an `X-Forest-Bff-Key` it answers `403 oauth_required`. |
| `openapiEnabled` | `boolean` | `false` | Serve the OpenAPI document over HTTP. Off by default when embedded. |
...
The secrets, the Forest URLs and the logger are inherited from the agent and cannot be overridden. These options are the only configuration channel: embedded, the BFF never reads the environment, so `BFF_RATE_LIMIT_MAX_REQUESTS`, `BFF_RATE_LIMIT_WINDOW_MS`, `BFF_PUBLIC_URL` and `HTTP_PORT` are ignored silently and their defaults stand. The feature switches default to off, so what you leave out stays off, and `GET /bff/health` reports which surfaces are on under `configured`.

The BFF answers under `/bff` on whatever port the agent is mounted on, so `POST /bff/agent/v1/{collection}/list`, `GET /bff/health`. A host serving the agent under a sub-path of its own works too, and `app.use('/api', mounted)` puts the BFF at `/api/bff`.

<Warning>
**Mount the agent before any body parser of your own.** A parser that runs first has already consumed the request stream, and nothing downstream can put it back, so every BFF `POST` answers `500 stream.not.readable`.
...
NestJS needs no special handling as long as you mount before `listen()`.
</Warning>

<Warning>
**Forest's IP whitelist does not apply to requests served under `/bff`.** They reach the agent in-process, over what the whitelist treats as a trusted loopback caller. That is deliberate, since applying the whitelist would filter the browsers a third-party UI is made of, and the agent logs a warning at startup naming the exemption. Authentication is still required, either an OAuth sign-in or an `X-Forest-Bff-Key` API key.
</Warning>

<Note>
`addBff()` and `mountAiMcpServer({ basePath: '/bff' })` are mutually exclusive: the MCP server would claim `/bff/oauth` and `/bff/mcp`. Whichever you call second throws on the spot, at the builder call rather than from `start()`. Any `basePath` landing inside `/bff` counts, so mount the MCP server elsewhere.
</Note>

`addBff()` can be called once, and before `start()`. A mount is what opens a socket, and `start()` only builds the agent's router, so without a `mountOn*` call nothing is served, the BFF included. While the BFF is not serving, `/bff/*` answers `503`, with a different type at each end of the lifecycle: `bff_not_started` between `addBff()` and the end of `start()`, `bff_stopped` after `stop()`.
