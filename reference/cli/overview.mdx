...
Copy a layout between environments through a file, for example to populate a new environment, which starts with the default layout.
...
Each run covers one environment and one team, so repeat it per team (`--force` skips the confirmation). `--with-workflows` includes workflow definitions.

<Warning>
Keep the colon: `forest layout pull`, with a space, prints help and exits successfully without doing anything.
</Warning>
...
| `forest teams:copy-layout` | Copy one team's layout onto another in the same project, overwriting it |

```bash
forest roles:copy --from production --to staging
```

`roles:copy` runs once per pair of environments, however many teams you have.
