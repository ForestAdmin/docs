...
description: "Common issues and solutions when setting up Forest or connecting your agent"
---

This guide covers common problems you might encounter when onboarding to Forest or connecting your agent to the platform.

## Agent can't connect to Forest UI

**Symptoms**: Your agent starts successfully, but the Forest UI shows "Agent not responding" or "Connection failed".

### Check outbound HTTPS access

Your agent needs to reach Forest's servers. Verify your firewall allows outbound connections to:
...
The `FOREST_ENV_SECRET` environment variable must match the secret provided in your Forest project settings. A mismatch will prevent authentication.
...
**Symptoms**: "Invalid token" or "Token expired" errors when accessing Forest UI.

### Solution

Log out and log back in to Forest:
...
**Symptoms**: Changes to your database schema don't appear in Forest UI, or collections are missing.

### Force schema refresh

The agent automatically detects schema changes, but a refresh is available on demand:
...
**Symptoms**: Created a new environment in Forest, but it doesn't show up in the UI or agent can't connect to it.
...
**Symptoms**: Browser console shows CORS errors when accessing Forest UI.

### Check agent URL configuration

Ensure your agent URL is correctly configured in Forest project settings:
...
### For outbound connections to Forest
...
2. **Search our community**: Browse the [Forest Community](https://community.forestadmin.com) for similar issues
3. **Contact support**: Reach out to our support team with:
   - Your agent logs (redact sensitive information)
   - Error messages you're seeing
   - Steps to reproduce the issue
   - Your Forest project ID
