...
Whether you have any renaming to do depends on what the old agent read, not on your language:

- **`forest-rails`** published the database column name, so a Rails project often has nothing to change here.
- **`forest-express-sequelize`** published the *Sequelize attribute* name, which parts company with the column as soon as a model uses `underscored` or an explicit `field` mapping. A `firstName` attribute over a `first_name` column was exposed as `firstName`.

[`datasource-sql`](/get-started/connect/data-sources/sql) introspects the database, so it exposes `first_name` either way. Where the two disagree, restore the old names explicitly:
