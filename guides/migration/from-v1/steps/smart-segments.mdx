...
Many queries can be expressed directly as Forest Admin condition trees, which gives much better performance than performing the query yourself and then building a naive `id IN (...)` condition.
...
<Tabs>
  ...
  <Tab title="After (Python)">
    ...
    agent.customize_collection("Products").add_segment(
        "best_sellers", best_sellers_segment_handler
    )
    ```
  </Tab>
</Tabs>
